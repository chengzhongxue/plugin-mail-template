...
public class NotificationTemplateValidator {
    ...
    private static String formatError(String partName, TemplateEngineException error) {
        TemplateProcessingException located = null;
        TemplateProcessingException deepest = null;
        Throwable current = error;
        while (current != null) {
            if (current instanceof TemplateProcessingException candidate) {
                deepest = candidate;
                if (candidate.hasLineAndCol()) {
                    located = candidate;
                }
            }
            current = current.getCause();
        }
        var processingError = located != null ? located : deepest;

        var location = "";
        if (located != null) {
            location = "（第 %d 行，第 %d 列）".formatted(
                located.getLine(), located.getCol());
        }
